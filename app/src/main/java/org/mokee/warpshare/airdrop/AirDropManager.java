...
//                thumbnail = AirDropThumbnailUtil.generate(entities.get(0));
                mMainThreadHandler.post(() -> {
//                    if (!thumbnailCanceled.get()) {
//                        ask(ref, peer, entities, listener);
//                    }
                });
            });
        } else {
    ...
    void handleAsk(String ip, NSDictionary request, AirDropServer.ResultCallback callback) {
        ...
        for (NSObject file : files) {
            ...
            if (fileTypeNode != null && filePathNode != null) {
                fileTypes.add(getMimeType((String) fileTypeNode.toJavaObject()));
                filePaths.add((String) filePathNode.toJavaObject());
            }
        }
        ...
        final String id = (String) idNode.toJavaObject();
        final String name = (String) nameNode.toJavaObject();

        Bitmap icon = null;
        final NSObject iconNode = request.get("FileIcon");
        if (iconNode != null) {
            try {
                final byte[] data = (byte[]) iconNode.toJavaObject();
